...
function renderPhaseDetail(phase) {
    ...
    // 检查 name 而不是 id，因为 quick 模式下 Implementation 的 id 是 2
    const isImplementation = phase.name && phase.name.toLowerCase().includes('implementation');
    if (isImplementation && typeof renderStoryExecutionPanel === 'function') {
