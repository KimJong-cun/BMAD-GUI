...
def find_sprint_status_file(project_path: Path) -> Path | None:
    """查找 Sprint 状态文件，支持多种目录命名格式"""
    # 先用 glob 模糊匹配 sprint 相关目录
    md_dir = project_path / "md"
    if md_dir.exists():
        for pattern in ["sprint*", "Sprint*"]:
            for sprint_dir in md_dir.glob(pattern):
                if sprint_dir.is_dir():
                    status_file = sprint_dir / "sprint-status.yaml"
                    if status_file.exists():
                        return status_file
        # 直接在 md 目录下查找
        direct = md_dir / "sprint-status.yaml"
        if direct.exists():
            return direct

    # 根目录查找
    root_file = project_path / "sprint-status.yaml"
    if root_file.exists():
        return root_file
